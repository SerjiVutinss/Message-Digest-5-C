...
void startMD5Hash(FILE *inFile) {
    ...
    while (processNextBlock(&M, inFile, &numBits, &status)) {
...
int processNextBlock(MessageBlock *M, FILE *infile, size_t *numBits, enum HashStatus *status) {
    ...
    *numBits += numFileBytesRead * 8;
    ...
    if (*status == PAD_ZEROES) {
        // Pad with zeroes.
        for (int i = 0; i < 57; i++) M->eight[i] = 0;
    ...
    if (numFileBytesRead == 64) return 1;
    // In this case, append the one-bit, pad with zeroes and then append the length.
    else if (numFileBytesRead < 56) {
        // Append the one-bit
        M->eight[numFileBytesRead] = 0x80;
        // Pad with correct number of zeroes...
        for (int i = numFileBytesRead + 1; i < 56; i++) M->eight[i] = 0;
        ...
        *status = FINISHED;
        ...
        for (int i = numFileBytesRead + 1; i < 64; i++) M->eight[i] = 0;
        // And set Status
        *status = PAD_ZEROES;
        return 1;
    }

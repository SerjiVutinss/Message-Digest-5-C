...
WORD *startMD5HashData(HashOptions hashData)
{
    ...
    {
        printf("FOUND FILE");
        return startMD5HashFile(hashData.file);
    }
...
WORD *startMD5StringHash(char *inputString)
{
    ...
    while (processStringBlock(&M, inputString, &numBits, &status))
    {
    ...
    // for (int i = 0; i < 4; i++)
    // {
    //     printf("%02x%02x%02x%02x", (H_MD5[i] >> 0) & 0xFF, (H_MD5[i] >> 8) & 0xFF, (H_MD5[i] >> 16) & 0xFF, (H_MD5[i] >> 24) & 0xFF);
    // }
    // printf("\n");
...
WORD *startMD5HashFile(FILE *inFile)
{
    ...
    // While there are still bytes in the file, keep processing blocks as needed.
    while (processFileBlock(&M, inFile, &numBits, &status))
    {
        // Hash the current block, passing in both the Block and the MD5 initialisers.
        nextHash(&M, H_MD5);
    }
...
//     while(processStringBlock(&M, buffer, &numBits, &status)) {
//         size_t bytesRead = fread(&M->eight, 1, 64, infile);
//         nextHash(&M, H_MD5);
//     }

//     int bytesInString = getBytesFromString(buffer, string, 0, 64);

//     printf("Read %d bytes from string\n", bytesInString);
